 ...
 *
 * What it does:
 *  This program starts with two particles right next to each other.  The
 * particles move through a three-dimensional phase space governed by the
 * following equations: dx/dt = sigma * ( y - x ) dy/dt = r * x - y + x * z
 *       dz/dt = x * y + b * z
 *  These are the Lorenz equations and define the "Lorenz Attractor."  Any two
 * particles arbitrarily close together will move apart as time increases, but
 * their tracks are confined within a region of the space.
 *
 * Commands:
...
double s0 = 10.0, r0 = 28.0,
       b0 = 8.0 / 3.0;   /* Default Lorenz attactor parameters */
double time_step = 0.03; /* Time step in the simulation */
double sigma = 10.0, ry = 28.0, b = 8.0 / 3.0; /* Lorenz attactor parameters */
...
double xcen = 0.0, ycen = 0.0,
       zcen = 0.0; /* Coordinates of the point looked at */
...
void calc_deriv(double position[3], double deriv[3]) {
  ...
  deriv[1] = (ry + position[2]) * position[0] - position[1];
  deriv[2] = -position[0] * position[1] - b * position[2];
}

void advance_in_time(double time_step, double position[3],
                     double new_position[3]) {
  /* Move a point along the Lorenz attractor */
  double deriv0[3], deriv1[3], deriv2[3], deriv3[3];
  int i;
  memcpy(new_position, position,
         3 * sizeof(double)); /* Save the present values */
  ...
  for (i = 0; i < 3; i++)
    new_position[i] =
        position[i] +
        0.1666666666666666667 * time_step *
            (deriv0[i] + 2.0 * (deriv1[i] + deriv2[i]) + deriv3[i]);
}

static void checkedFGets(char *s, int size, FILE *stream) {
...
void key_cb(unsigned char key, int x, int y) {
  ...
  case 'M': /* Modify the Lorenz parameters */
    printf("Please enter new value for <sigma> (default %f, currently %f): ",
           s0, sigma);
    checkedFGets(inputline, sizeof(inputline), stdin);
    sscanf(inputline, "%lf", &sigma);

    printf("Please enter new value for <b> (default %f, currently %f): ", b0,
           b);
    checkedFGets(inputline, sizeof(inputline), stdin);
    sscanf(inputline, "%lf", &b);

    printf("Please enter new value for <r> (default %f, currently %f): ", r0,
           ry);
    checkedFGets(inputline, sizeof(inputline), stdin);
    sscanf(inputline, "%lf", &ry);
...
void timer_cb(int value) {
  ...
  if (animate > 0) {
    if (new_index == NUM_POINTS)
      new_index = 0;
    advance_in_time(time_step, red_position[array_index],
                    red_position[new_index]);
    advance_in_time(time_step, grn_position[array_index],
                    grn_position[new_index]);
...
int main(int argc, char *argv[]) {
  ...
  /* Initialize the attractor:  The easiest way is to call the keyboard callback
   * with an argument of 'r' for Reset.
   */
  key_cb('r', 0, 0);
